...
def getSlideType(h4Title, h4ConElem):
    ...
    # h5 태그가 있는지로 타입 크게 분류
    headerCnt =  h4ConElem.count('<h5')

    # h5가 있을 경우: (h5헤딩,h5내용) 튜플들의 리스트를 슬라이드 타입의 1st 인자로
    h5Tuples = []
    if headerCnt>0:
    ...
    # h5가 없을 경우: '불렛하나내용'이나 '엔터친거한줄내용' 스트링들의 리스트를 슬라이드 타입의 1st 인자로
